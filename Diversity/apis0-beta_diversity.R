...
metadata2 <- read.delim("apis0-metadata.tsv", sep = "\t", header = T, quote = "", stringsAsFactors = F)
metadata2[1,]
metadata2[,1]
...
metadata<-read_q2metadata("apis0-metadata.tsv")
str(metadata)
levels(metadata$`Caste`)
colnames(metadata)[2] <- "Caste"
colnames(metadata)[3] <- "Colony"
str(metadata)

row.names(metadata) <- metadata[,1]
row.names(metadata) <- metadata$SampleID
metadata_wo_col1 <- metadata[,-1]
row.names(metadata)

bc_PCoA <- read_qza("bray_curtis_pcoa_results.qza")
wUF <- read_qza("weighted_unifrac_pcoa_results.qza")

Castes <- c("Black", "Blue", "Green", "Gray", "Red")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Caste)) +
  geom_point() + #alpha controls transparency and helps when points are overlapping
  theme_q2r() +
  xlab("PC1 (49.43%)") +
  ylab("PC2 (22.99%)") +
  scale_color_manual(values=c("Blue", "Black", "Green", "Gray", "Red"), name = "Caste")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=Castes, name = my_column)
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Caste)) +
  geom_point(aes(shape= Colony)) + #alpha controls transparency and helps when points are overlapping
  geom_point(data=centroids, size = 3) +
  theme_q2r() +
  stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*bc_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) 
  scale_color_manual(values=Castes, name = my_column)
ggsave(paste0("output/BC-ellipse_", my_column,"-caste.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

#just the centroids plotted
ggplot(bc_meta, aes(x=PC1, y=PC2, color=Caste)) +
  geom_point(aes(shape= Colony)) + #alpha controls transparency and helps when points are overlapping
  geom_point(data=centroids, size = 3) +
  theme_q2r() +
...
ggsave(paste0("output/BC-ellipse_", my_column,"-castecentroids.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color= get(my_column))) +
  ...
  scale_color_manual(values=Castes, name = "Caste")
ggsave(paste0("output/Wuni-ellipse_", my_column,".pdf"), height=3, width=4, device="pdf") # save a PDF 3 inches by 4 inches

ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Colony), size = 3) + #alpha controls transparency and helps when points are overlapping
  geom_point(data= centroids, size = 3) +
  theme_q2r() +
  stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*Wuni_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=Castes, name = "Caste")
ggsave(paste0("output/Wuni-ellipse_", my_column,"-caste.pdf"), height=3, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(jaccard_meta, aes(x=PC1, y=PC2, color= get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=Castes, name = "Caste")
ggsave(paste0("output/jaccard-ellipse_", my_column,".pdf"), height=3, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(jaccard_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=Castes, name = "Caste")
ggsave(paste0("output/jaccard-ellipse_", my_column,"-caste.pdf"), height=3, width=4, device="pdf") # save a PDF 3 inches by 4 inches
...
PERMANOVA_out <- adonis2(bc_dm ~ Caste, data = metadata_sub)

write.table(PERMANOVA_out,"output/Caste_Adonis_overall.csv",sep=",", row.names = TRUE) 
...
Caste_Pair <- pairwise.adonis2(bc_dm ~ Caste, data = metadata_sub)
write.table(Caste_Pair,"output/Caste_Adonis_pairwise.csv",sep=",", row.names = TRUE) 
...
ggplot(UWuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=Castes, name = "Caste")
ggsave(paste0("output/UWuni-ellipse_", my_column,"-caste.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
